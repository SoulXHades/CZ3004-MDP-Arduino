...
3. Watch out ypur battery's voltage. Get a multimeter. Always measure the voltage. Find the ideal voltage. Calibrate to that ideal voltage and do a run only at that ideal voltage. Ideally, only 2 runs are possible before the voltage drops even more and the robot's movement and readings get affected.
